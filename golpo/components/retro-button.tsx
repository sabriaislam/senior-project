...
export function RetroButton(props: AsButton | AsLink) {
  if ("href" in props && props.href !== undefined) {
    const { className = "", ...rest } = props as AsLink;
    return <Link className={`retro-btn ${className}`.trim()} {...rest} />;
  }
  const { className = "", ...rest } = props as AsButton;
  return <button className={`retro-btn ${className}`.trim()} {...rest} />;
}
